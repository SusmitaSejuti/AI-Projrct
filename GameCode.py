...
class MahjongGame(object):
    ...
    def is_legal_move(self, row, col, vertical):
        if vertical:
            selected_cells = ((row, col), (row+1, col))
        else:
            selected_cells = ((row, col), (row, col+1))

        if not self.move_on_board(selected_cells):
            return False

        if not self.move_on_free_space(selected_cells):
            return False

        return True
    
    def is_legal_move2(self, row, col, vertical):
        if vertical:
            selected_cells = ((row, col), (row-1, col))
        else:
            selected_cells = ((row, col), (row, col-1))
    ...
    def is_legal_move3(self, row, col, vertical):
        selected_cells = ((row, col), (row-1, col-1))
        if not self.move_on_board(selected_cells):
            return False
        if not self.move_on_free_space(selected_cells):
            return False
    ...
    def is_legal_move4(self, row, col, vertical):
    ...
    def is_legal_move5(self, row, col, vertical):
    ...
    def is_legal_move6(self, row, col, vertical):
        selected_cells = ((row, col), (row+1, col+1))    
        if not self.move_on_board(selected_cells):
            return False
    ...
    def move_on_board(self, selected_cells):
        for square in selected_cells:
            row = square[0]
            col = square[1]
    ...
    def move_on_free_space(self, selected_cells):
        for square in selected_cells:
            row = square[0]
            col = square[1]
    ...
    def perform_move2(self, row, col, vertical):
        if vertical:
            selected_cells = ((row, col), (row-1, col))
        else:
            selected_cells = ((row, col), (row, col-1))
    ...
    def perform_move3(self, row, col, vertical):
        selected_cells = ((row, col), (row-1, col-1))
        for square in selected_cells:
            row = square[0]
            col = square[1]
    ...
    def perform_move4(self, row, col, vertical):
    ...
    def perform_move5(self, row, col, vertical):
    ...
    def perform_move6(self, row, col, vertical):
    ...
    def copy(self):
        return MahjongGame(copy.deepcopy(self._board))
